...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes(String stringUrl) {
        ...
        try {
            ...
            for (int i = 0; featuresArray.length() > i; i++){
                JSONObject item = featuresArray.getJSONObject(i);
    ...
    private static String makeHttpRequest(URL url) throws IOException{
        ...
        try {
            ...
            // set up http connection params
            httpURLConnection.setRequestMethod("GET");
            httpURLConnection.setConnectTimeout(10000);
            httpURLConnection.setReadTimeout(15000);
            // make the connection
            httpURLConnection.connect();
            // check connection if it was successful
            if(httpURLConnection.getResponseCode() == 200){

                // get the data as input stream
                inputStream = httpURLConnection.getInputStream();
                // parse the input stream using the method readInputStream
                jsonResponse = readInputStream(inputStream);
            }else{

                Log.e(LOG_TAG, "Error in reponse code not 200 "+httpURLConnection.getResponseCode());
            }

        }catch (IOException e){
            Log.e(LOG_TAG, "IOException in makeHttpRequest "+e.getMessage());
        }finally {
    ...
    private static String readInputStream(InputStream inputStream) throws IOException{
        ...
        if(inputStream != null){
            ...
            // use InputStreamReader to read and convert the bytes into words
